...
<div class="first">
	...
	<div class="first-txt">
		<h1>بەشێ بینینا فورمان </h1>
	    <p>ئەڤ بەشە تایبەتە ب بینینا فورمێن پەردا، هیڤیدارین ئاگەهداربەو دەستکاریێ نەکە.</p>
	    <center><button><a href="done_show_curtains.php">
	    بینینا فۆرمێن دۆماهیک هاتین </a></button></center>
	</div>
</div>
<div class="show-curtains">
	<?php
	$null = null;
    $query = mysqli_query($db , "SELECT * FROM `add_curtains` WHERE `done` = '$null' ORDER BY `id` DESC ");
			...
			<div class="show-curtains-info">
				<div class="show-curtains-info-header">
				    ...
				    <center>
				    	...
				    	<button style="float: right;" data-bs-toggle="modal" data-bs-target="#edit<?php echo $row['id'];?>">دۆماهیک</button>
				    	<button type="delete" style="background-color: red; float: left;"><a style="color: white;" href="modal/curtains.inc.php?delete=<?php echo $row['id'];?>">ژێبرن   </a></button>
				    </center>
			</div>
		</div>
	<?php } ?>
...
<!-- MODAL EDIT -->
<?php 
$query = mysqli_query($db , "SELECT * FROM `add_curtains` ORDER BY `date` DESC");
while($row = mysqli_fetch_assoc($query)){?>
