...
<div class="table-data">
	<table width="90%">
		...
		<?php
		$sum = 0;
		$query = mysqli_query($db , "SELECT * FROM `ayad` ORDER BY `id` ASC");
		...
		<tr>
			...
			<td><button style="background-color: red;"><a style="color: white;" href="modal/user.inc.php?delete_ayad=<?php echo $row['id'];?>">ژێبرن   </a></button></td>
		</tr>
...
<div class="table-data">
	...
	<div class="month-data-cards">
		<?php
		$date = date("Y/m/d");
		$query = mysqli_query($db , "SELECT MONTHNAME(day) AS mname, sum(money) AS total FROM `ayad` GROUP BY MONTHNAME(day)");
		while($row = mysqli_fetch_assoc($query)){ ?>
